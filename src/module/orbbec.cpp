...
struct Orbbec::state_ {
    std::string resource_name;
    std::string serial_number;

    explicit state_(std::string serial_number, std::string resource_name) : serial_number(serial_number), resource_name(resource_name) {}
};
std::mutex state_mu_;

std::unique_ptr<struct Orbbec::state_> Orbbec::configure_(vsdk::Dependencies dependencies, vsdk::ResourceConfig configuration) {
...
Orbbec::Orbbec(vsdk::Dependencies deps, vsdk::ResourceConfig cfg)
    : Camera(cfg.name()), state_(configure_(std::move(deps), std::move(cfg))) {
