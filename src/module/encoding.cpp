...
std::vector<std::uint8_t> encode_to_png(std::uint8_t const * const image_data, int const width, int const height, ImageFormat const format) {
    ...
    int color_type;
    switch (format) {
        case ImageFormat::GRAY: color_type = PNG_COLOR_TYPE_GRAY; break;
        case ImageFormat::RGB: color_type = PNG_COLOR_TYPE_RGB; break;
        case ImageFormat::RGBA: color_type = PNG_COLOR_TYPE_RGBA; break;
        default:
            png_destroy_write_struct(&png_ptr, &info_ptr);
